...
def recompute_progress(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["RowProgress%"] = df["Etape"].map(PROGRESS_MAP).fillna(0.0)
    df["CompletedMass_Row"] = df["TOT MASS (Kg)"] * df["RowProgress%"]
    return df
...
# -------------------------------------------------
    ...
    def filter_view(_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if search_asm.strip():
            ...
            _view = _view[
                _view["ASSEMBLY NO."].astype(str).str.lower().str.contains(pat)
                | _view["PART NO."].astype(str).str.lower().str.contains(pat)
            ]
        return _view
    ...
    with sub_tab_items:
        ...
        if st.button("🔄 Synchroniser (pièces)", key="sync_items_btn"):
            ...
            st.session_state["df"] = (
                st.session_state["df"]
                .drop(columns=["Etape"])
                .merge(updated_map, on=key_cols, how="left")
                .assign(Etape=lambda x: x["Etape"].fillna("None"))
            )
            st.session_state["df"] = recompute_progress(st.session_state["df"])
            st.success("✅ Synchronisation effectuée (pièces → global)")
        if st.button("🔄 Actualiser le tableau des pièces", key="refresh_items"):
            view_items = filter_view(st.session_state["df"])
            st.success("✅ Tableau des pièces mis à jour")
    ...
    with sub_tab_asm:
        ...
        if search_asm.strip():
            pat = search_asm.strip().lower()
            df_asm_view = df_asm_view[
                df_asm_view["ASSEMBLY NO."].astype(str).str.lower().str.contains(pat)
            ]
...
with tab_graph:
    ...
    df_steps["Cumul_Masse"] = df_steps["CompletedMass"].cumsum()
    df_steps["Cumul_%"] = df_steps["Avancement%"].cumsum().clip(upper=100)
    fig_s = px.line(
        df_steps,
        x="Etape",
        y=["Cumul_Masse", "Cumul_%"],
        title="Diagramme S — Masse cumulée & % cumulatif",
        labels={"value": "Cumul", "Etape": "Étape"},
        markers=True
    )
...
# -------------------------------------------------
    ...
    c1, c2 = st.columns(2)
    with c1:
        if st.button("💾 Enregistrer (backup + écrasement du fichier local)", type="primary"):
            ...
            try:
                ...
                st.success(f"✅ Sauvegardé. Backup créé : {backup_name}")
            except Exception as e:
